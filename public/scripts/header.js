(function() {
    ...
    function initHover() {
      ...
      function detectActive() {
        ...
        navItems.forEach(function(item) {
          var link = item.matches('a, button[data-dropdown-trigger]')
            ? item
            : item.querySelector('a, button[data-dropdown-trigger]');
          if (!link) return;
  ...
  function doSearch() {
    ...
    loadSearchIndex().then(function (index) {
      ...
      var matched = (index || []).filter(function (p) {
        return (p.title || '').toLowerCase().indexOf(lower) !== -1
          || (p.excerpt || '').toLowerCase().indexOf(lower) !== -1
          || (p.category || '').toLowerCase().indexOf(lower) !== -1
          || (p.tags || []).some(function (t) { return String(t).toLowerCase().indexOf(lower) !== -1; })
          || (p.content || '').toLowerCase().indexOf(lower) !== -1;
      }).slice(0, 20);
